...
import {make, Bitmap, Context, encodePNGToStream} from '../src/index.js';
import fs from 'fs';
...
describe('draw curve',() => {
    ...
    it('bezier curve', (done) => {
        ...
        c.lineTo(10,10);
        c.fill();
        encodePNGToStream(image, fs.createWriteStream(path.join(DIR,'bezier1.png'))).then(() => {
    ...
    it('arc', (done) => {
        ...
        encodePNGToStream(img, fs.createWriteStream(path.join(DIR,'arc.png'))).then(()=>{
    ...
    it('north going polygon', (done) => {
        ...
        ctx.lineTo(20,50);
        ctx.fill();
        encodePNGToStream(img, fs.createWriteStream(path.join(DIR,'northgoing.png'))).then(()=>{
    ...
    it('draws thick square',(done) => {
        ...
        // c.fillStyle = 'black'
        // c.fill()
        encodePNGToStream(image, fs.createWriteStream(path.join(DIR,'thick_stroke_square.png'))).then(() => {
    ...
    it('draws thick curve',(done) => {
        ...
        // c.fillStyle = 'black'
        // c.fill()
        encodePNGToStream(image, fs.createWriteStream(path.join(DIR,'thick_stroke_curve.png'))).then(() => {
            console.log('wrote out thick_stroke.png');
            // expect(image.getPixelRGBA(0, 0)).to.eq(WHITE)
            // expect(image.getPixelRGBA(19, 39)).to.eq(BLACK)
            done();
