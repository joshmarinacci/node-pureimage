 ...
 * Logical Not
 */
export const not = function (uint32val: number) {
  return ~uint32val >>> 0;
};
...
export const rotateLeft = function (uint32val: number, numBits: number) {
  return (
    (((uint32val << numBits) >>> 0) | (uint32val >>> (32 - numBits))) >>> 0
  );
};

export const rotateRight = function (uint32val: number, numBits: number) {
  return (
 ...
 */
export const choose = function (x: number, y: number, z: number) {
  return ((x & (y ^ z)) ^ z) >>> 0;
};
 ...
 */
export const majority = function (x: number, y: number, z: number) {
  return ((x & (y | z)) | (y & z)) >>> 0;
};
 ...
 */
export const addMod32 = function (uint32val: number, ...argv: number[]) {
